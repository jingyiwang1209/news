...
class PythonDatabaseInterface:
    """The class contains methods of connecting, closing a database and processing a query."""
...
q1 = view+"select public.articles.title, count(*) as nums from "\
     "public.articles join subsq on public.articles.slug = "+formatted_path+ \
     "group by public.articles.title order by nums desc limit 3;"
...
q2 = "create view viewer as select public.articles.author, "\
     "count(subsq.id) as nums from public.articles left join subsq "\
     "on public.articles.slug = "+formatted_path+ \
     "group by public.articles.author;"\
     "select public.authors.name, viewer.nums from public.authors, viewer "\
     "where public.authors.id = viewer.author order by viewer.nums desc;"
...
view_error= "create view errors as select time::timestamp::date, count(status) "\
            "as error_count from public.log where not status = '200 OK' "\
            "group by time::timestamp::date;"
...
view_final = "create view final as select total.time, "\
             "(errors.error_count::float /total.total_count::float)*100 as prob "\
             "from total right join errors on total.time = errors.time;"
